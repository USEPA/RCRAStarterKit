...
It has 4 tools:  
1 - Update to RCRA Schema - This takes the input data and converts it to the EPA RCRA data model schema. It outputs a new data layer.  This tool also runs Get Event Data and Get Evaluation Data.  Tool 2 and 3 do not need to be run if you run this tool.
...
Note:  If your study area contains EventCodes, separate your data into two files: one for polygons with events, and another for polygons without events. Event require additional fields that cannot have nulls.  Splitting the data will help avoid import errors when the GeoJSON is imported in the RCRA website.
